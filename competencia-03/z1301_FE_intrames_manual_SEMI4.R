...
AgregarVariables_IntraMes <- function(dataset) {
  ...
  # Crear la función para sumar ignorando valores nulos
  suma_sin_na <- function(col1, col2) {
    return(rowSums(cbind(col1, col2), na.rm = TRUE))
  }
  
  # Crear las nuevas columnas sumando las existentes
  dataset[, mconsumo := suma_sin_na(mtarjeta_visa_consumo, mtarjeta_master_consumo)]
  dataset[, mfinanciacionlimite := suma_sin_na(Master_mfinanciacion_limite, Visa_mfinanciacion_limite)]
  dataset[, msaldopesos := suma_sin_na(Master_msaldopesos, Visa_msaldopesos)]
  dataset[, msaldodolares := suma_sin_na(Master_msaldodolares, Visa_msaldodolares)]
  dataset[, mconsumopesos := suma_sin_na(Master_mconsumospesos, Visa_mconsumospesos)]
  dataset[, mconsumodolares := suma_sin_na(Master_mconsumosdolares, Visa_mconsumosdolares)]
  dataset[, mlimitecompra := suma_sin_na(Master_mlimitecompra, Visa_mlimitecompra)]
  dataset[, madelantopesos := suma_sin_na(Master_madelantopesos, Visa_madelantopesos)]
  dataset[, madelantodolares := suma_sin_na(Master_madelantodolares, Visa_madelantodolares)]
  dataset[, madelantos := suma_sin_na(madelantopesos, madelantodolares)]
  dataset[, mpagado := suma_sin_na(Master_mpagado, Visa_mpagado)]
  dataset[, mpagospesos := suma_sin_na(Master_mpagospesos, Visa_mpagospesos)]
  dataset[, mpagosdolares := suma_sin_na(Master_mpagosdolares, Visa_mpagosdolares)]
  dataset[, msaldototal := suma_sin_na(Master_msaldototal, Visa_msaldototal)]
  dataset[, mconsumototal := suma_sin_na(Master_mconsumototal, Visa_mconsumototal)]
  dataset[, cconsumostotal := suma_sin_na(Master_cconsumos, Visa_cconsumos)]
  dataset[, morosidad := suma_sin_na(Master_delinquency, Visa_delinquency)]
  
  # Calcular la suma directa de múltiples columnas para `ctransacciones`
  dataset[, ctransacciones := rowSums(.SD, na.rm = TRUE), .SDcols = c("ctarjeta_debito_transacciones", "ctarjeta_visa_transacciones", "ctarjeta_master_transacciones")]
