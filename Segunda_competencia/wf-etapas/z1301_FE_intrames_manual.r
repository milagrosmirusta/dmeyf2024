...
AgregarVariables_IntraMes <- function(dataset) {
  ...
  # Variables de cambio estacional mes a mes
  if (atributos_presentes(c("mcuentas_saldo"))) {
    dataset[, diff_saldo := mcuentas_saldo - shift(mcuentas_saldo, n = 1, type = "lag")]
  }
  if (atributos_presentes(c("ctarjeta_debito_transacciones", "ctarjeta_visa_transacciones", "ctarjeta_master_transacciones"))) {
    dataset[, ctransacciones := ctarjeta_debito_transacciones + ctarjeta_visa_transacciones + ctarjeta_master_transacciones]
    dataset[, diff_transacciones := ctransacciones - shift(ctransacciones, n = 1, type = "lag")]
  }
  
  # Promedios y variaciones en ventanas de tiempo
  if (atributos_presentes(c("mcuentas_saldo"))) {
    dataset[, saldo_3_meses := frollmean(mcuentas_saldo, n = 3, align = "right")]
    dataset[, saldo_6_meses := frollmean(mcuentas_saldo, n = 6, align = "right")]
    dataset[, saldo_12_meses := frollmean(mcuentas_saldo, n = 12, align = "right")]
  }
  if (atributos_presentes(c("ctransacciones"))) {
    dataset[, transacciones_3_meses := frollmean(ctransacciones, n = 3, align = "right")]
    dataset[, transacciones_6_meses := frollmean(ctransacciones, n = 6, align = "right")]
    dataset[, transacciones_12_meses := frollmean(ctransacciones, n = 12, align = "right")]
  }
  
  # Cambio porcentual en ventanas de 6 meses para saldo y transacciones
  if (atributos_presentes(c("mcuentas_saldo"))) {
    dataset[, perc_change_saldo_6_meses := (mcuentas_saldo - shift(mcuentas_saldo, n = 6, type = "lag")) / shift(mcuentas_saldo, n = 6, type = "lag")]
  }
  if (atributos_presentes(c("ctransacciones"))) {
    dataset[, perc_change_trans_6_meses := (ctransacciones - shift(ctransacciones, n = 6, type = "lag")) / shift(ctransacciones, n = 6, type = "lag")]
  }
  
  # Variables de tendencia estacional
  if (atributos_presentes(c("foto_mes"))) {
    dataset[, es_diciembre := month(foto_mes) == 12]
    dataset[, trimestre_4 := quarter(foto_mes) == 4]
  }
  
  # Índice de variación estacional para saldo y transacciones
  if (atributos_presentes(c("mcuentas_saldo", "foto_mes"))) {
    dataset[, annual_mean_saldo := mean(mcuentas_saldo, na.rm = TRUE), by = year(foto_mes)]
    dataset[, indice_saldo := mcuentas_saldo / annual_mean_saldo]
  }
  if (atributos_presentes(c("ctransacciones", "foto_mes"))) {
    dataset[, annual_mean_transacciones := mean(ctransacciones, na.rm = TRUE), by = year(foto_mes)]
    dataset[, indice_transacciones := ctransacciones / annual_mean_transacciones]
  }
